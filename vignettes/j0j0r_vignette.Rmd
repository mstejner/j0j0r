...
$$
f(p_\perp, p_parallel) =\frac{n}{(2\pi)^3 p_{\perp\,T}^2 p_{\parallel\,T}} e^{(-((p_\parallel - p_{drift})^2 /p_{\parallel\,T}^2 + p_\perp^2 / p_{\perp\,T}^2))}
$$
...
```{r, echo = FALSE, results = FALSE, fig.width = 16, fig.height = 8}
...
```{r, echo = TRUE, results = FALSE}
j0j0r::j0j0_examples %>% 
  dplyr::filter(particle %in% c("maxwellian", "wilkie_b0", "wilkie_b5")) %>% 
  ggplot2::ggplot(
    mapping = ggplot2::aes(
      x = frequency/1e6, 
      y = Re(j0j0) + Im(j0j0), 
