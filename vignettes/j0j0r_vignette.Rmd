...
In the exaples above, $\langle \widetilde{\bf{j}}^{(a0)} \widetilde{\bf{j}}^{(a0)}\rangle$, was seen to display ion cyclotron features when $\phi$ is close to $90^\circ$ and for the ring and slowdown distributions - both of which have highly localized maxima and/or edges in velocity space.
...
$$\langle \widetilde{\bf{j}}^{(a0)} \widetilde{\bf{j}}^{(a0)}\rangle = (2\pi)^2 \frac{m_a q_a^2}{\lvert k_\parallel \rvert} \sum_{l = -\infty}^\infty \int dp_\perp p_\perp  {\bf c}_l^\strut {\bf c}_l^* f^{(a0)}(p_\perp, p_\parallel)$$
...
```{r fig13, echo = FALSE, fig.width = 16, fig.height = 8, fig.align = "center"}
pp2 <- dist_2D_examples %>% 
...
pp3 <- dist_2D_examples %>% 
  ...
  ggplot2::ggtitle("Slowdown distribution, b = 5") +
    ggplot2::geom_hline(
    yintercept = v_parallel[abs(v_parallel) < 1.5e6], 
    linetype = "dashed", 
    color = "black",
    size = 1.5
    ) +
...
These arguments can be generalized. When there is a highly localized feature, shuch as a well defined maximum or a sharp edge, in the velocity distribution, and when $\delta v_\parallel = \omega_c/k_\parallel$ is sufficiently large, it matters a great deal whether the value of $\omega$ allows that feature in the distribution to be probed. The result for $\widetilde{\bf{j}}^{(a0)} \widetilde{\bf{j}}^{(a0)}\rangle$ can then depend strongly on the precise value of $\omega$. 

Additional complexity arises from the factor $p_\perp  {\bf c}_l^\strut {\bf c}_l^*$ that multiply the distribution in the integrals. The figure below shows examples of its dependence on $l$ and $\p_\perp$ with similar assumptions as above. 
...
df[["factor"]] <- df[["clcl"]] * p_perp
df[["l"]] <- as.factor(df[["l"]])
df[["direction"]] <-  paste0(df[["direction"]], df[["direction"]])

ggplot2::ggplot(
  data = df, 
  mapping = aes(x = v_perp, y = factor, color = l)
  ) +
  geom_line(size = 1.5) +
  scale_y_log10() +
  ...
  ylab(unname(latex2exp::TeX("abs(p_{perp}  c_l c_l^*)"))) +
  xlab(latex2exp::TeX("$v_{perp}$ in m/s^2")) +
  scale_x_continuous(labels = scales::scientific)
```

The Bessel functions have roots at irregular intervals, and at those roots the factor $p_\perp  {\bf c}_l^\strut {\bf c}_l^*$ goes rapidly to zero leading to narrow minima. This will hardly matter, if integral at many different values of $l$ contribute to the sum - the combined effect will still lead to a smooth dependence on frequency. But, when $\delta v_\parallel = \omega_c/k_\parallel$ is sufficiently large, so only a few of the integrals provide significamt contributions, it makes a big difference if one of those roots happens to coincide with a maximum in the velocity distribution. For example, the slowdown distribution with $b=5$ is stronly dominated by the narrow peak near the birhc velocity. It is then quite important if the contribution from that peak is suppressed by a root in $p_\perp  {\bf c}_l^\strut {\bf c}_l^*$. Thus it is possible for narrow localized features in the velocity distribution to cause both maxima and minima in $\widetilde{\bf{j}}^{(a0)} \widetilde{\bf{j}}^{(a0)}\rangle$.
