...
The code assumes that the distributions are always in cylindrical coordinates and symmetric about the magnetic field, so ${\bf p = (p_\perp, p_\parallel)$ where the magnetic field is in the z-direction so $p_\perp =\sqrt{(p_x^2 + p_y^2)}$ and $p_\parallel = p_z$. When transforming between different coordinate systems the distribution function must remain normalized, so
...
Note that with this convention, the factor $2\pi p_\perp$ is considered part of the volume element, and not of the distribution function $f(p_\perp, p_\parallel)$. So with this convention the expression for a Maxwellian velocity distribution is 
...
j0j0r::calculate_distribution_data_frame(
  ...
  viridis::scale_fill_viridis(option="plasma", limits = c(0, 1.5e82)) +
